...
title: "Land use land cover change due to energy transition-induced 
        resource extraction in Cabo Delgado, Mozambique"
...
### Loading the boundaries of the projects and their surrounding areas

The resource extracting projects (Twigg, GK and Total) are mainly hosted in 
three administrative posts (Balama, Ancuabe and Palma). 

Therefore, our areas of interest (aoi) include: the administrative posts, 
the boundaries of the projects, and the areas surrounding the projects, that is, 
including the project affected communities (PACs). Of these, our principal 
study areas are the boundaries of the projects and their surroundings.

We acquired the relevant shapefiles from various sources including:-
- Administrative posts and location of communities: In Mozambique, Eduardo 
  Mondlane University and the Ministry of Transport and Communication; 
  specifically the Spatial Development Program (SDP). Similar shapefiles can be 
  acquired from online open access sources such as GRID: https://data.grid3.org/
- Boundaries of mines: https://portals.landfolio.com/mozambique/en/ 
...
### loading the satellite images of our areas of interest

To study the change in land use and land cover (lulc) in our areas of interest, 
between 2015 to 2021, we use and therefore load bands 1 to 7 of the Landsat 8 
satellite. We also load the qa bands which indicate the pixels within the 
...
To prepare the images for analysis, we create and use a cloud mask to mask out 
...
the levels of our administrative posts of interest.
...
### Create training samples for image classification

To classify the images of our areas of interest, we created training samples 
from RGB composites of the areas. To aid with distinguishing features, we also 
used ndvi layers, Google Earth (https://earth.google.com/web/), and published 
projects' environmental impact assessment reports:
...
We created approximately 500 pixels per image. This process was conducted in 
ArcGIS Pro Version 2.8.3, but it can also be done in open access QGIS 
(https://www.qgis.org/en/site/).

Where possible, training samples can (and for increased accuracy should be) 
generated from groundtruth data. However, given the wide extent of the study 
areas, and the inaccessibility of some of these areas due to an armed conflict, 
creation of training samples from the images was most feasible. The generation 
of training samples from imagery as a surrogate for ground observation has been 
utilized in other cases (Foody, 2002).
...
From the training sample polygons, we select sample points for use in the 
lulc classification process. The spectral profiles of the samples can be 
inspected to ensure adequate differentiation among the lulc classes. We follow 
the workflow suggested by Ghosh & Hijmans (2021).
...
We classify each image using the random forest model and crop the output by the
three study areas and by the boundaries of the three projects. 
...
LULCdf_fn <- function(stdyarea_lulc) {
  for (i in 1:length(stdyarea_lulc)) {
    LULCdf              <- stdyarea_lulc
...
The cropped outputs include a 'count' column which includes area in pixels 
taken up by each lulc class. We multiply these areas by 0.0009 to convert it to 
square kilometers. We use the changes in these lulc areas for each location 
over the two time stamps (2021/2020 & 2015) to determine the change in lulc due 
to energy transition-induced resource extraction in Cabo Delgado, Mozambique.
...
lulc_acc <- function(accuracies) {
  ...
  colnames(confmat)  <- lulc_df$class #TODO specify class per location
  samples            <- sum(confmat)
