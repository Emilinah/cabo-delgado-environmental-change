...
aois_clean_bandnames <- function(aois_list) {
  bandlist <- aois_list
  names(bandlist[[1]][[1:7]]) <- gsub(pattern = ".*SR",
                                   replacement = names(bandlist[1]),
                                   names(bandlist[[1]][[1:7]]))
  names(bandlist[[2]][[1:7]]) <- gsub(pattern = ".*SR",
                                   replacement = names(bandlist[2]),
                                   names(bandlist[[2]][[1:7]]))
  names(bandlist[[3]][[1:7]]) <- gsub(pattern = ".*SR",
                                   replacement = names(bandlist[3]),
                                   names(bandlist[[3]][[1:7]]))
  names(bandlist[[4]][[1:7]]) <- gsub(pattern = ".*SR",
                                   replacement = names(bandlist[4]),
                                   names(bandlist[[4]][[1:7]]))
  names(bandlist[[5]][[1:7]]) <- gsub(pattern = ".*SR",
                                   replacement = names(bandlist[5]),
                                   names(bandlist[[5]][[1:7]]))
  names(bandlist[[6]][[1:7]]) <- gsub(pattern = ".*SR",
                                   replacement = names(bandlist[6]),
                                   names(bandlist[[6]][[1:7]]))
  return(bandlist)
}
...
aois_comps <- list("blma2021_b543" = comps(aois_final[[1]], 5, 4, 3),
                   "blma2015_b543" = comps(aois_final[[2]], 5, 4, 3),
                   "blma2021_b654" = comps(aois_final[[1]], 6, 5, 4),
                   "blma2015_b654" = comps(aois_final[[2]], 6, 5, 4),
                   "ancbe2020_b543" = comps(aois_final[[3]], 5, 4, 3),
                   "ancbe2015_b543" = comps(aois_final[[4]], 5, 4, 3),
                   "ancbe2020_b654" = comps(aois_final[[3]], 6, 5, 4),
                   "ancbe2015_b654" = comps(aois_final[[4]], 6, 5, 4),
                   "plma2020_b543" = comps(aois_final[[5]], 5, 4, 3),
                   "plma2015_b543" = comps(aois_final[[6]], 5, 4, 3),
                   "plma2020_b654" = comps(aois_final[[5]], 6, 5, 4),
                   "plma2015_b654" = comps(aois_final[[6]], 6, 5, 4))
aois_comps

aois_compsDummy <- aois_comps[1:2]
aois_compsDummy # start here - saving
aois_compsDummy[[1]]

X <- writeRaster(aois_compsDummy$blma2021_b543,
                 file.path("~/results/output", names(aois_compsDummy[[1]])),
                 overwrite=TRUE,
                 filetype="GTiff")
X
