...
LULC <- list("blma2021_LULC" = crop(aois_lulc[[1]], sa[[1]]),
             "blma2015_LULC" = crop(aois_lulc[[2]], sa[[1]]),
             "ancb2021_LULC" = crop(aois_lulc[[3]], sa[[2]]),
             "ancb2015_LULC" = crop(aois_lulc[[4]], sa[[2]]),
             "plma2021_LULC" = crop(aois_lulc[[5]], sa[[3]]),
             "plma2015_LULC" = crop(aois_lulc[[6]], sa[[3]]))
LULC
...
LULC_freqs <- function(cover) {
  for (i in 1:length(cover)) {
  lulc_freq <- freq(cover[[i]])*0.0009
  return(lulc_freq)
}
}
LULC_freqSA <- LULC_freqs(LULC)

LULC_freqsMns <- function(cover_mines) {
  for (i in 1:length(cover_mines)) {
  lulc_freqMns <- freq(cover_mines[[i]])*0.0009
  return(lulc_freqMns)
}
}
LULC_freqMines <- LULC_freqsMns(lulc_mines)
LULC_freqMines ##??

# plot in arcgis pro

ggplot() +
  geom_tile(data = blma2021_classfddf_nna, aes(x = x, y = y, fill = class_value)) +
